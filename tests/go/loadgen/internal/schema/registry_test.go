...
func TestRegisterProtobufReturnsID(t *testing.T) {
	const protoText = `syntax="proto3"; message M {}`
	...
	id, err := RegisterProtobuf(context.Background(), srv.URL, "t-value", protoText)
	if err != nil {
		t.Fatal(err)
	}
	if id != 42 {
