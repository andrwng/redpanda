...
class NodeOperationFuzzyTest(EndToEndTest):
    ...
    def setUp(self):
        super(NodeOperationFuzzyTest, self).setUp()
    ...
    def generate_random_workload(self, count, available_nodes):
        ...
        def decommission(id):
            ...
            if id in maintenance_nodes:
                # We shouldn't have to worry about exiting maintenance mode for
                # this node anymore.
        ...
        def start_maintenance(id):
            assert len(maintenance_nodes) == 0, \
                "Should only schedule one maintenance node at a time"
            active_nodes.remove(id)
            maintenance_nodes.append(id)

        def stop_maintenance(id):
            assert len(maintenance_nodes) > 0, \
                f"Scheudled stop maintenance on {id} when no nodes are in maintenance"
            active_nodes.append(id)
            maintenance_nodes.remove(id)
    ...
    def perform_work(self, work):
        ...
        def decommission(idx):
            ...
            def decommissioned():
                try:
                    ...
                    # Wait for the node to be completely decommissioned.
                    brokers = admin.get_brokers()
                    for b in brokers:
                        if b['node_id'] == node_id:
                            _ = admin.decommission_broker(id=node_id)
                            return False
                    return True
        ...
        def check_maintenance(admin, node, expect_maintenance):
            node_id = self.ids_mapping[idx]
            try:
                status = admin.maintenance_status(node, node_id=node_id)
        ...
        def start_maintenance(idx):
            ...
            admin.maintenance_start(node, node_id=node_id)
        ...
        def stop_maintenance(idx):
            ...
            def stop_maintenance():
                try:
                    admin.maintenance_stop(node, node_id=node_id)
                    return True
                except requests.HTTPError:
                    # It's possible the node is down because of an injected
                    # failure.
                    return False

            wait_until(stop_maintenance, timeout_sec=10, backoff_sec=1)
    ...
    @cluster(num_nodes=5)
    def test_stop_maintenance_after_decommission(self):
        work = []
        work.append((START_MAINTENANCE, 1))
        work.append((DECOMMISSION, 1))
        work.append((STOP_MAINTENANCE, 1))
        self.perform_work(work)

    @cluster(num_nodes=5)
    def test_new_maintenance_after_decommission(self):
        work = []
        work.append((START_MAINTENANCE, 1))
        work.append((DECOMMISSION, 1))
        work.append((START_MAINTENANCE, 2))
        self.perform_work(work)
