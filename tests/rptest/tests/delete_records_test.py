...
class DeleteRecordsTest(RedpandaTest):
    ...
    def test_delete_records_with_transactions(self):
        ...
        # Assert the logic that evicts only up until the max collectible
        # offset was actually invoked during the test
        search_logs_with_timeout(self.redpanda,
                                 "Cannot prefix truncate due to")
    ...
    def test_delete_records_concurrent_truncations(self):
        ...
        # Restart a node, with 1s of downtime
        node = random.choice(self.redpanda.nodes)
        self.redpanda.signal_redpanda(node, signal=signal.SIGKILL)
        time.sleep(1)
        self.redpanda.start_node(node)
