...
class DatalakeClusterRestoreTest(RedpandaTest):
    ...
    def __init__(self, test_context: TestContext):
        ...
        super(DatalakeClusterRestoreTest,
    ...
    def n_avro_schema(self, n: int):
        ...
        avro_fields = [
            f'{{"name": "{f_name}", "type": "int"}}'
            for f_name in self.n_field_names(n)
        ]
        avro_schema = f'''
        {{
            "type": "record",
    ...
    def restore_cluster_and_wait(self, topics: list[str]):
        ...
        wait_until(lambda: self.has_all_topics(topics),
                   timeout_sec=60,
                   backoff_sec=1,
    ...
    def start_structured_topic_streams(
            ...
            create_schemas: bool = True) -> tuple[list[str], list[str]]:
        """
    ...
    def offsets_per_table(self, dl: DatalakeServices,
    ...
    def test_basic(self, cloud_storage_type, catalog_type) -> None:
        ...
        with DatalakeServices(self.test_context,
            ...
            assert num_schemas_records > 0, "Expected schemas topic to have been restored"

            # Restart our workloads without creating the schemas or topics
            # explicitly.
            self.start_all_topic_streams(dl,
    ...
    @matrix(cloud_storage_type=supported_storage_types(),
            catalog_type=supported_catalog_types())
    def test_restore_partition_spec(self, cloud_storage_type,
                                    catalog_type) -> None:
        """
        ...
        with DatalakeServices(self.test_context,
            ...
            def check_expected_psec():
                for t in all_topics:
                    expected_partition_line = ('redpanda.partition', 'int',
                                               None)
    ...
    @matrix(cloud_storage_type=supported_storage_types(),
            catalog_type=supported_catalog_types())
    def test_slow_tiered_storage_dlq(self, cloud_storage_type,
                                     catalog_type) -> None:
        """
        ...
        with DatalakeServices(self.test_context,
            ...
            num_schemas_records = self.get_num_schemas_records()
            assert num_schemas_records == 0, "Expected schemas topic to not have been restored"
            ...
            for s in structured_streams:
                self.rpcn.stop_stream(s, should_finish=None)
            self.start_structured_topic_streams(dl,
                                                create_topics=False,
                                                create_schemas=True)
    ...
    @matrix(cloud_storage_type=supported_storage_types(),
            catalog_type=supported_catalog_types())
    def test_slow_tiered_storage_dupe_records(self, cloud_storage_type,
                                              catalog_type) -> None:
        """
        Test when tiered storage is explicitly slower than the translation
        ...
        with DatalakeServices(self.test_context,
            ...
            spark = dl.spark()
            for t in all_topics:
                dupes_query = "select redpanda.offset, count(*) as counts " \
