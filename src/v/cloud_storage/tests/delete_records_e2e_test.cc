...
FIXTURE_TEST(test_delete_local, delete_records_e2e_fixture) {
    ...
    // XXX: DeleteRecords above the max collectible offset does not work.
    // XXX: when in the same batch, start offset is not honored.
    // XXX: the offset is sent to cloud as is, but the limitation about batch
    // boundaries is still true.
    for (const auto& r : records) {
        producer.produce_to_partition(topic_name, model::partition_id(0), {r})
          .get();
    }
    // producer.produce_to_partition(topic_name, model::partition_id(0),
    // records)
    //   .get();
    log->flush().get();
