...
struct instance_generator<cluster::prepare_group_tx_reply> {
    ...
    static std::vector<cluster::prepare_group_tx_reply> limits() {
        return {
          cluster::prepare_group_tx_reply(cluster::tx_errc::none),
          cluster::prepare_group_tx_reply(cluster::tx_errc::invalid_txn_state),
        };
    }
};
