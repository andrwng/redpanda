...
avro::Schema partition_type_to_schema(const struct_type& partition_type) {
    return struct_type_to_avro(partition_type, "r102");
}
...
avro::Schema data_file_schema(const struct_type& partition_type) {
    ...
    schema.addField(
      "partition", partition_type_to_schema(partition_type), field_attrs(102));
...
std::unique_ptr<struct_value> data_file_to_value(data_file file) {
    ...
    ret->fields.emplace_back(std::move(file.partition.val));
...
struct_value manifest_entry_to_value(manifest_entry entry) {
    ...
    if (entry.snapshot_id.has_value()) {
        ret.fields.emplace_back(long_value(*entry.snapshot_id));
    } else {
        ret.fields.emplace_back(std::nullopt);
    }
    if (entry.sequence_number.has_value()) {
        ret.fields.emplace_back(long_value(*entry.sequence_number));
    } else {
        ret.fields.emplace_back(std::nullopt);
    }
    if (entry.file_sequence_number.has_value()) {
        ret.fields.emplace_back(long_value(*entry.file_sequence_number));
    } else {
        ret.fields.emplace_back(std::nullopt);
    }
    ret.fields.emplace_back(data_file_to_value(std::move(entry.data_file)));
    return ret;
}
