...
ss::sstring content_type_to_str(manifest_content_type t) {
    switch (t) {
...
manifest_content_type content_type_from_str(std::string_view s) {
    return string_switch<manifest_content_type>(s)
      .match("data", manifest_content_type::data)
      .match("deletes", manifest_content_type::deletes);
}

ss::sstring format_to_str(format_version v) {
    switch (v) {
...
format_version format_from_str(std::string_view s) {
    return string_switch<format_version>(s)
      .match("1", format_version::v1)
      .match("2", format_version::v2);
}

struct partition_spec_strs {
    ss::sstring spec_id_str;
    ss::sstring fields_str;
};
...
metadata_to_map(const manifest_metadata& meta) {
    ...
    return {
      ...
      {"content", content_type_to_str(meta.manifest_content_type)},
      {"partition-spec", partition_spec_strs.fields_str},
      {"partition-spec-id", partition_spec_strs.spec_id_str},
      {"format-version", format_to_str(meta.format_version)}};
}
...
metadata_from_reader(avro::DataFileReader<manifest_entry>& rdr) {
    ...
    m.partition_spec = partition_spec_from_str(partition_spec_strs{
      ...
      .fields_str = find_required_str("partition-spec"),
    });
