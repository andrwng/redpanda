...
TEST(PositionDeleteFile, RoundTrip) {
    ...
    deletes.push_back(
      {.file_path = uri{"s3://bucket/data/file_a.parquet"}, .pos = 0});
    deletes.push_back(
      {.file_path = uri{"s3://bucket/data/file_a.parquet"}, .pos = 42});
    deletes.push_back(
      {.file_path = uri{"s3://bucket/data/file_b.parquet"}, .pos = 7});
...
TEST(PositionDeleteFile, RoundTripCompressed) {
    chunked_vector<position_delete_entry> deletes;
    deletes.push_back(
      {.file_path = uri{"s3://bucket/data/file.parquet"}, .pos = 100});
