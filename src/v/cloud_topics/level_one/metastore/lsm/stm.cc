...
ss::future<lsm_stm_snapshot> stm::make_snapshot() const {
    ...
    snapshot.state = co_await state_.copy();
    co_return snapshot;
}
...
void stm::rearm_snapshot_timer() {
    ...
    auto delay = snapshot_delay_secs_();
    if (delay.count() > 0) {
        snapshot_timer_.rearm(ss::lowres_clock::now() + delay);
    }
}

void stm::write_snapshot_async() {
    ssx::spawn_with_gate(_gate, [this] { return maybe_write_snapshot(); });
}

ss::future<> stm::maybe_write_snapshot() {
    if (!_raft->is_leader() || _is_catching_up) {
        co_return;
    }
    co_await metastore_stm_base::write_local_snapshot();
    rearm_snapshot_timer();
}
