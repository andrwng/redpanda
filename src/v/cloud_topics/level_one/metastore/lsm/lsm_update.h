...
struct apply_write_batch_update
  : public serde::envelope<
    ...
    operator==(const apply_write_batch_update&, const apply_write_batch_update&)
      = default;
    auto serde_fields() { return std::tie(rows); }

    static constexpr auto key{lsm_update_key::apply_write_batch};

    static std::expected<apply_write_batch_update, lsm_update_error>
    build(const lsm_state&, chunked_vector<write_batch_row> rows);
...
struct persist_manifest_update
  : public serde::envelope<
    ...
    static constexpr auto key{lsm_update_key::persist_manifest};

    static std::expected<persist_manifest_update, lsm_update_error>
    build(const lsm_state&, domain_uuid, lsm::proto::manifest manifest);
    std::expected<std::monostate, lsm_update_error>
    can_apply(const lsm_state&) const;
...
struct set_domain_uuid_update
  : public serde::envelope<
    ...
    static constexpr auto key{lsm_update_key::set_domain_uuid};
...
struct reset_manifest_update
  : public serde::envelope<
    ...
    static std::expected<reset_manifest_update, lsm_update_error>
    build(const lsm_state&, domain_uuid, std::optional<lsm::proto::manifest>);
    std::expected<std::monostate, lsm_update_error>
    can_apply(const lsm_state&) const;
    std::expected<std::monostate, lsm_update_error>
